...
FLOW = {
    ...
    # Ввод VIN (только ждём текст и валидируем)
    "t_vin": {
        "text": "Введи VIN (17 символов, латиница/цифры, без I,O,Q):",
        "expect": "text",
        "validator": "is_vin",
        "var_name": "vin",
        "next": "t_vin_sizes",              # ← и это
    },

    # Промежуточный шаг: дергаем экшен и идём к списку размеров
    "t_vin_fetch": {
        "text": "Проверяю VIN…",
        "action": "tires_sizes_from_vin",   # <-- здесь вызываем поиск
        "next": "t_vin_sizes",
    },

    # Выбор размера, найденного по VIN (кнопки подставим динамически)
    "t_vin_sizes": {
        "text": "По VIN найдены размеры. Выбери один:",
        "buttons": [],
        "next_by_button": {}
    },
}
